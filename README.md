...
Subsequent requests include the cookie and JwtAuthenticationFilter validates it.

User identity and role are set in the SecurityContext.
...
Login as each user and save JWT token from cookie.

Seller: Add new products and update details.

Buyer: Add items to cart, update quantities, place orders.

Test protected endpoints to verify role-based access.

Send invalid JSON to test validation errors.

Test invalid IDs or unauthorized actions to verify error handling.
